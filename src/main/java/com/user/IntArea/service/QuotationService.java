...
public class QuotationService {
    ...
    public Quotation rejectQuotation(QuotationRequest quotationRequest) {
        ...
        quotation.setQuotationProgress(QuotationProgress.CUSTOMER_CANCELLED);
    ...
    public Quotation approveQuotation(QuotationRequest quotationRequest) {
        ...
        quotation.setQuotationProgress(QuotationProgress.CUSTOMER_APPROVED);
        return quotationRepository.save(quotation);
    }
    ...
    public void create(QuotationCreateDto quotationCreateDto) {
        ...
        Quotation quotation = Quotation.builder()
                ...
                .quotationProgress(QuotationProgress.SELLER_CREATED_AND_PENDING)
                .build();
        quotationRepository.save(quotation);

        // 이미지파일 첨부 로직
        List<MultipartFile> multipartFiles = new ArrayList<>();
        multipartFiles.add(quotationCreateDto.getImageFiles());
        TestMultiImageDto testMultiImageDto = new TestMultiImageDto();
        testMultiImageDto.setImages(multipartFiles);
        imageController.multiTest(testMultiImageDto);
    }

    // (seller) 작성한 견적서 취소처리 => isAvailable = false;
    public Quotation cancelQuotation(Quotation quotation) {
        quotation.setAvailable(false);
        quotation.setQuotationProgress(QuotationProgress.SELLER_CANCELLED);
        return quotationRepository.save(quotation);
    }
    ...
    public void updateQuotation(QuotationUpdateDto quotationUpdateDto) {
        ...
        if(formerQuotation.isPresent()){
            ...
            formerQuotation.get().setQuotationProgress(QuotationProgress.SELLER_UPDATED_FORMER_QUOTATION_REMAINS_AS_RECORD);
        }
        ...
        Quotation newQuotation = Quotation.builder()
                ...
                .quotationRequest(quotationRequest.get())
                .quotationProgress(QuotationProgress.SELLER_UPDATED_AND_PENDING)
                .build();

        // 이미지파일 첨부 로직
        List<MultipartFile> multipartFiles = new ArrayList<>();
        multipartFiles.add(quotationUpdateDto.getImageFiles());
        TestMultiImageDto testMultiImageDto = new TestMultiImageDto();
        testMultiImageDto.setImages(multipartFiles);
        imageController.multiTest(testMultiImageDto);
    ...
    public void makeRejectionContractQuotation(QuotationRequest quotationRequest) {
        ...
        if(optionalFormerQuotation.isPresent()) {
            ...
            if(formerQuotation.getQuotationProgress().equals(QuotationProgress.SELLER_CANCELLED)) {
                throw new NoSuchElementException("이미 취소한 견적입니다.");
            }

            // 최초 견적서 생성 후 고객의 응답 대기중이거나, 고객이 견적서 수정을 요청했거나, 판매자가 견적서 수정 후 고객의 응답 대기중일 경우에만 거래 취소 가능
            if(formerQuotation.getQuotationProgress().equals(QuotationProgress.SELLER_CREATED_AND_PENDING) ||
                    formerQuotation.getQuotationProgress().equals(QuotationProgress.CUSTOMER_ASKED_REVISE) ||
                    formerQuotation.getQuotationProgress().equals(QuotationProgress.SELLER_UPDATED_AND_PENDING)
            ) {
                formerQuotation.setAvailable(false);
                formerQuotation.setQuotationProgress(QuotationProgress.SELLER_CANCELLED);
            }
        ...
        Quotation newQuotation = Quotation.builder()
                ...
                .quotationProgress(QuotationProgress.SELLER_CANCELLED)
