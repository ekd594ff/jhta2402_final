...
const ReviewList = ({ companyId }) => {
    ...
    const fetchReviewsBasedOnRole = async (companyId, page, size) => {
        try {
            // 권한을 확인하는 API 호출
            const roleResponse = await axios.get('/api/member/role', { withCredentials: true });
            ...
            // Debugging log
            console.log('Total Reviews:', response.data.page.totalElements);
            console.log('Loaded Reviews:', response.data.content.length);
            ...
            // 평균 평점 계산
            const totalRating = [...reviews, ...newReviews].reduce((acc, review) => acc + review.rate, 0);
            const avgRating = totalRating / (reviews.length + newReviews.length);
            setAverageRating(Math.round(avgRating * 100) / 100); // 소숫점 둘째 자리에서 반올림
    ...
    return (
        <>
            ...
            <Box sx={{ display: 'grid', gap: 2, gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))' }}>
                {reviews.map((review) => (
                    <Card key={review.id} sx={{ padding: 2, borderRadius: 2, boxShadow: 2 }}>
                        <CardContent>
